...
class Excel(View):
    def get(self, request, *args, **kwargs):
        ...
        for row_num, cls in enumerate(schedule, 2):
            ...
            meeting_time = f"{cls.meeting_time.pid}, {cls.meeting_time.day}, {cls.meeting_time.time}"

            data_row = [section_id, dept, course, room, instructor, meeting_time]

            for col_num, value in enumerate(data_row, 1):
                column_letter = get_column_letter(col_num)
                sheet[f"{column_letter}{row_num}"] = value
                sheet[f"{column_letter}{row_num}"].alignment = Alignment(horizontal="center")
